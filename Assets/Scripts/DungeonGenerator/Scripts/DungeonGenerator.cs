...
public class DungeonGenerator : MonoBehaviour {
    ...
    public void BuildStandartDungeon()
    {
        data = Dungeondata;
        targetRooms = LevelModel.Dungeon.Stage.stageRoomsCount[LevelModel.Dungeon.Stage.currentDungeonCount];
        Create();
    ...
    public void RecostructNavigarionSurface(){
        StartCoroutine("buildnavmesh");
    }
    ...
    if (rooms.Count!= 0){
      ...
      //  if(room.GetComponent<Room>().CountedRoom){
            roomsCount++;
        Debug.Log("fuck:" + room.name);
       // }
    ...
    private void GenerateNextRoom() {
        ...
        if (!roomIsGood) {
           ...
           // roomsCount++;
            ...
            if(newRoom.GetComponent<Room>().CountedRoom){
                FuckRooms++;
            }
